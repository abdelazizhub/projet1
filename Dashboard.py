...
#***************************************************#
def prepocessing_var(df, type_scal):
    from sklearn.preprocessing import MinMaxScaler, StandardScaler
    df_scaled = df.copy()
    ...
    if type_scal == 'minmax':
        scal = MinMaxScaler()
    elif type_scal == 'standard':
        scal = StandardScaler()
    else:
        raise ValueError("Type de scaling inconnu. Utilisez 'minmax' ou 'standard'.")

    # Sélection des colonnes de type float64
    cols = df.select_dtypes(include=['float64']).columns

    # Vérifier que des colonnes existent et que le DataFrame n'est pas vide
    if len(cols) > 0 and not df[cols].empty:
        df_scaled[cols] = scal.fit_transform(df[cols])
    else:
        print("Aucune colonne de type float64 trouvée pour le scaling ou DataFrame vide.")
...
def valeur_shape(client_id):
    try:
        response = requests.get(f"{API_URL}shaplocal/{client_id}", timeout=10)
        ...
        if response.status_code != 200:
            ...
            try:
                ...
                error_msg += f": {error_data.get('error', '')} - {error_data.get('details', '')}"
            except:
        ...
        required_keys = ['shap_values', 'base_value', 'data', 'feature_names']
        if not all(k in data for k in required_keys):
            st.error("Structure de données SHAP incomplète")
            return None

        # Conversion des types
        try:
            return shap.Explanation(
                values=np.array(data['shap_values']).reshape(1, -1),
                base_values=data['base_value'],
                data=np.array(data['data']).reshape(1, -1),
                feature_names=data['feature_names']
            )
        except Exception as e:
            st.error(f"Erreur de conversion SHAP: {str(e)}")
            return None
...
def df_voisins(id_client):
    ...
    from io import StringIO  # À placer en haut de votre fichier si ce n'est pas déjà fait
    data_voisins = pd.read_json(response.json())
    print(data_voisins.columns)
    return data_voisins
...
if page == "Interprétation locale":
    ...
    if locale:
        if id_client_dash != '<Select>':
            with st.spinner('Calcul des explications locales...'):
                shap_explanation = valeur_shape(id_client_dash)
            
                if shap_explanation:
                    try:
                        plt.figure(figsize=(10, 6))
                        nb_features = st.slider('Nombre de variables à visualiser', 0, 20, 10)
                        shap.plots.waterfall(shap_explanation[0], max_display=nb_features, show=False)
                        st.pyplot(plt.gcf())
                        plt.clf()
                    except Exception as e:
                        st.error(f"Erreur d'affichage : {str(e)}")
                else:
                    st.warning("Aucune donnée disponible pour l'affichage SHAP")
        else:
            st.warning("Veuillez sélectionner un client")
...
if page == "Interprétation globale":
    ...
    if globale:
        ...
        data_test_std = prepocessing_var(data_test.drop('SK_ID_CURR', axis=1), 'standard')
        nb_features = st.slider('Nombre de variables à visualiser', 0, 20, 10)
        fig, ax = plt.subplots()
        # Affichage du summary plot : shap global
        shap_values_class = shap_values[1]

# Vérifier si c'est un vecteur et le transformer en matrice 2D si nécessaire
        if shap_values_class.ndim == 1:
            shap_values_class = np.atleast_2d(shap_values_class)

        ax = shap.summary_plot(shap_values_class, data_test_std, plot_type='bar', max_display=nb_features)
        st.pyplot(fig)
